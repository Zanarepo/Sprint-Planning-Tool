...
export default function EstimationPlayground() {
  ...
  // User & saved estimation states (userEmail removed to fix ESLint warning)
  const [userId, setUserId] = useState(null);
  const [savedEstimations, setSavedEstimations] = useState([]);
  // For editing an entire saved estimation table
  ...
  // Retrieve user email from localStorage, then fetch user id, activities, and saved estimations
  useEffect(() => {
    ...
    if (email) {
      const fetchUserData = async () => {
        ...
        // Fetch saved estimations for the user
  ...
  // Function to fetch saved estimations for a user
  ...
  // Create (Save) a new estimation table (batch)
  const saveEstimation = async () => {
    if (!userId) {
      console.error("User ID not found. Cannot save estimation.");
      return;
    }
    // Generate a new batch_id and friendly batch name
    ...
    if (error) {
      ...
      // Reset the current estimation inputs
      setFeatures([]);
      setEstimates({});
  ...
  // Update the entire estimation table (batch) using the data in editingBatchData
  // This function recalculates total_estimate from the updated features and estimates.
  const updateEstimationFull = async (recordId) => {
    if (!editingBatchData) return;
    // Parse features (assumed to be newline separated) and estimates (assumed to be valid JSON)
    ...
    // Recalculate total_estimate based on the selected method
  ...
  // Delete an entire estimation batch (all rows with the given batch_id)
  ...
  // Group saved estimations by batch_id (each group represents one saved table)
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="p-4 border rounded space-y-4 shadow-sm">
            ...
            <div className="space-y-2">
              ...
              <p>
                Features per Sprint:{" "}
                {(features.length / (totalEstimate / velocity)).toFixed(1)}
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="container mx-auto px-0 mt-8 flex items-center gap-4">
        <button
      ...
      {/* Saved Estimations - Grouped by Batch with CRUD Ops */}
      <div className="container mx-auto px-0 mt-8">
        ...
        {Object.keys(groupedEstimations).length === 0 ? (
          ...
          Object.entries(groupedEstimations).map(([batchId, estimationsGroup]) => {
            // We assume each batch is represented by a single record
            const record = estimationsGroup[0];
            return (
              <div key={batchId} className="mb-8 border p-4 rounded shadow">
                {editingBatchId === record.id ? (
                  // Edit mode: show a form for the entire table (excluding velocity)
                  <div className="space-y-4">
                    <div>
                  ...
                  // Display mode: show table details with an Edit Table button
                    ...
                    <button
                      onClick={() =>
                        setEditingBatchData({
                          ...
                          // Pre-populate features and estimates as text:
        ...
        {activities.length === 0 ? (
          ...
          <ul className="list-disc ml-6">
            {activities.map((activity) => (
              <li key={activity.id}>
                <span className="font-medium">{activity.activity_text}</span> - {new Date(activity.created_at).toLocaleString()}
              </li>
            ))}
          </ul>
        )}
